# Added the save-ids option to obj_det_and_trk.
# Only works with videos. Ad-hoc modification to solve a specific problem.
# Should NOT be used. Use obj_det_and_trk instead.
...
def get_video_properties(videoPath):
    ...
    return fps,totalNoFrames,durationInSeconds
...
def detect(weights=ROOT / 'yolov5n.pt',
        ...
        blur_obj=False,color_box = False,save_ids=None):
    
    save_img = not nosave and not source.endswith('.txt') 
    
    # NEW: Init tracked IDs list
    allID=[]
    # Get number of frames (valid only if video)
    _,totalNoFrames,_=get_video_properties(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for track in tracks:
                    # NEW: Store the ID
                    allID.append([frame,track.detclass,track.id])
                    if color_box:
        ...
        if (i%10==0 or i>=totalNoFrames-1):
            progress_bar(frame,totalNoFrames)
    ...
    if not (save_ids is None):
        allID=np.array(allID).astype('int')
        np.savetxt(save_ids,allID,delimiter=',')
